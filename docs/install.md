...
Sidecar supports macOS and Linux. Windows isn't supported yet.
...
A repo can additionally pin sidecar as a node dependency. When it does, the
`sidecar` command always runs that project-local version, so a repo's pinned
sidecar owns its own behavior. Local sidecar never installs hooks or
background sync: it syncs only on an explicit `sidecar sync`, and its
postinstall clones a missing checkout and registers the repo with the global
daemon when one exists. (Older versions installed git hooks; current versions
remove those automatically.)

Daemon commands always run the global executable, never a project-local copy.
